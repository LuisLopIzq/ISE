...
osTimerId_t tim_id_1s;                            // timer id
osTimerId_t tim_id_3min;
osTimerId_t tim_id_2s;
/***********************/
...
void init_RTC(void){
  ...
  osTimerStart(tim_id_3min, 10000U);
 ...
 void RTC_CalendarConfig(uint8_t hora, uint8_t min, uint8_t sec, uint8_t year, uint8_t mes, uint8_t dia)
{
  ...
  /* Set Date: Saturdsay January 1th 2000 */
...
int Init_timers (void) {
	...
	tim_id_2s = osTimerNew((osTimerFunc_t)&Timer_Callback_2s, osTimerPeriodic, &exec, NULL);
}
